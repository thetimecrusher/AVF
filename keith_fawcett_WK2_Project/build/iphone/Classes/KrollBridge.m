...
NSString * keith_fawcett_WK2_Project$ModuleRequireFormat = @"(function(exports){"
...
@implementation keith_fawcett_WK2_ProjectObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_keith_fawcett_wk2_project);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create keith_fawcett_WK2_Project global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "keith_fawcett_WK2_Project" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_keith_fawcett_wk2_project = [[keith_fawcett_WK2_ProjectObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_keith_fawcett_wk2_project];
	
	NSString *_keith_fawcett_wk2_projectNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _keith_fawcett_wk2_projectNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_keith_fawcett_wk2_project valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:keith_fawcett_WK2_Project$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.keith_fawcett_wk2_project.kroll" 
