...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kkeith_fawcett_WK3_ProjectUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kkeith_fawcett_WK3_ProjectUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a keith_fawcett_WK3_Project app using the node.js CLI
