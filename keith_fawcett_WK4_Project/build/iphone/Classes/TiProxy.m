...
//Rare exceptions to indicate a bug in the _keith_fawcett_wk4_project code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_keith_fawcett_wk4_project = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_keith_fawcett_wk4_project]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.keith_fawcett_wk4_project.module"
